...
class MMBO(nn.Module):
    def __init__(self,num_classes=1):
        ...
        self.conv1 = nn.Conv2d(in_channels=5120, out_channels=2048, kernel_size=1, stride=1, padding=0)
        self.dropout = nn.Dropout(0.5)  # Dropout层，丢弃概率为0.1
        self.textlinear = nn.Linear(512,2048)
        self.textencode = text_encoder(device=device)
        self.relu = nn.ReLU()
    ...
    def forward_vector(self, x,position):
        ...
        for i in range(N):
            ...
            cnn_x = self.DAM(input_x,input_position) 
            cnn_x1 = self.QAM(input_x) 
            cnn_x1=cnn_x1.view(cnn_x1.size(0),-1)
            cnn_x1=self.relu(self.fc(cnn_x1))
            
            feature_dis = torch.cat((cnn_x, feature_dis),dim=1)
            feature_dis1 = torch.cat((cnn_x1, feature_dis1),dim=1)
    ...
    def forward(self, patch,position,text):
        ...
        feature_vertor_x,feature_vertor_x1 = self.forward_vector(patch,position)
        feature_vertor_x = self.conv1(feature_vertor_x)
        feature_vertor_x = self.Avgpool2d(feature_vertor_x)
        feature_vertor_x1 = self.conv1(feature_vertor_x1)
        feature_vertor_x1 = self.Avgpool2d(feature_vertor_x1)
        x = feature_vertor_x
        x1 = feature_vertor_x1
        x = x.view(x.size(0), -1)
        x1 = x1.view(x1.size(0), -1)
        i_t = torch.cat((x,x1),dim=1)
        i_t = self.fc1(i_t)
        att_image_to_text_features=self.compute_attention_weights(i_t,text_feature)
        weight_image_features=self.apply_attention_weights(text_feature,att_image_to_text_features)+text_feature
        i_t=torch.cat((i_t,weight_image_features),dim=1)
