...
class CLIP(nn.Module):
    ...
    def forward_vector(self, x):
        ...
        for i in range(N):
            input_x = x[:,i,:,:,:]
            cnn_x = self.encode_image(input_x)   
            # cnn_x=cnn_x.view(cnn_x.size(0),-1)
            # cnn_x=self.relu(self.fc(cnn_x))
            # cnn_x=self.conv(cnn_x)
            # cnn_x=cnn_x.view(cnn_x.size(0), -1)# torch.Size([4, 256, 28, 28])
            # cnn_x=self.relu(self.fc(cnn_x))

            # dis = self.backbone(input_x)

         
            # out_x = self.upsample(self.upsample(self.conv2(dis)))
    ...
    def forward(self, image, text_long):
        image_features_long = self.forward_vector(image)
        image_features_long = torch.stack(image_features_long,dim=1)
        image_features_long = torch.mean(image_features_long,dim=1)
        ...
        # image_features_short=image_features_short.type_as(text_features_short)
        # text_features_short=text_features_short.type_as(image_features_short)
        # 计算相似度
        sim_i2tl = torch.matmul(image_features_long, text_features_long.T)
        sim_tl2i = torch.matmul(text_features_long, image_features_long.T)
        sim_tl2i = sim_tl2i.T


        # sim_i2ts = torch.matmul(image_features_short, text_features_short.T)
        # sim_ts2i = torch.matmul(image_features_short, text_features_short.T)
        # sim_ts2i = sim_ts2i.T
