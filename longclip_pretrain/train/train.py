...
import sys
sys.path.append('/mnt/10T/zjy/D_OIQA/Long-CLIP-main')

print(sys.path)

from sharegpt4v import share4v_val_dataset, share4v_train_dataset,DE_t_Dataset
...
class CLIP_Clean_Train():
    def __init__(self, args):
        ...
        # self.model, _ = longclip.load("/home/iqateam/team/media1/ZJY/DOIQA/longclip_model/longclip-B.pt",device=device)
        self.model, _ = longclip.load_from_clip(self.base_model, device='cpu',download_root=args.download_root)
        self.model.load_state_dict(torch.load('/mnt/10T/zjy/D_OIQA/longclip_model/longclip-B.pt'))
        self.model.train()
        # print(self.model)
        # self.model.train()
        ...
        # self.model = torch.nn.parallel.DistributedDataParallel(self.model, device_ids=1)
    ...
    def train_epoch(self, dataloader, epoch, start_iter=0):
        ...
        for i, (images, texts) in enumerate(tqdm(dataloader)):
            ...
            with torch.amp.autocast('cuda'):
                loss_long = self.model(images, texts)#,loss_short
            # try:
    ...
    def train(self, resume=False, warmup_length=200):
        trainset = DE_t_Dataset(cfg=DE_360IQA_config())
        # train_sampler = DistributedSampler(dataset=trainset, shuffle=True)
        train_loader = torch.utils.data.DataLoader(trainset, batch_size=self.batch_size, num_workers=8, pin_memory=True,shuffle=True)
        ...
        for epoch in range(start_epoch, self.num_epoch):
            ...
            torch.save(self.model.state_dict(), '/mnt/10T/zjy/D_OIQA/Long-CLIP-main/checkpoints/SPAQ/'+formatted_date+name)
            # print("=====================================")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--weight_decay', default=1e-5, type=float, help='wd.')
    ...
    parser.add_argument(
        "--epochs", type=int, default=12, help="Number of epochs to train for."
    )
    parser.add_argument(
        "--resume",
        default=False,
